...
void main() {
  group("crypto.MnemonicCode", () {
    ...
    test("badLength", () {
      List<String> words =
          _split("risk tiger venture dinner age assume float denial penalty hello");
      expect(() => _mc.check(words), throwsA(new isInstanceOf<MnemonicException>()));
    });

    test("badWord", () {
      List<String> words =
          _split("risk tiger venture dinner xyzzy assume float denial penalty hello game wing");
      expect(() => _mc.check(words), throwsA(new isInstanceOf<MnemonicException>()));
    });

    test("badChecksum", () {
      List<String> words =
          _split("bless cloud wheel regular tiny venue bird web grief security dignity zoo");
      expect(() => _mc.check(words), throwsA(new isInstanceOf<MnemonicException>()));
    });
