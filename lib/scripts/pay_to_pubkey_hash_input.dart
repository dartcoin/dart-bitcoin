library bitcoin.scripts.input.pay_to_pubkey_hash;
...
class PayToPubKeyHashInputScript extends Script {
  ...
  factory PayToPubKeyHashInputScript(dynamic signature, dynamic pubKey) {
    ...
    return new PayToPubKeyHashInputScript.convert(
        new ScriptBuilder().data(signature).data(pubKey).build(), true);
  }

  PayToPubKeyHashInputScript.convert(Script script, [bool skipCheck = false])
  ...
  TransactionSignature get signature =>
      new TransactionSignature.deserialize(chunks[0].data, requireCanonical: false);
